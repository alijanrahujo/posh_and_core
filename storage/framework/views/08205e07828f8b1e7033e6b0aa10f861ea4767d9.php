    ...
    var table_table = $('#tax_type-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "<?php echo e(route('tax_type.index')); ?>",
                ...
                render: function (data,type,row) {
                    if (row.type == 'fixed') {
                        if ('<?php echo e(config('variable.currency_format') =='suffix'); ?>') {
                            return row.rate+ ' <?php echo e(config('variable.currency')); ?>';
                        } else {
                            return '<?php echo e(config('variable.currency')); ?> ' + row.rate;
                        }
                    } else {
                        return row.rate + '%';
                    }
    ...
    $('#tax_type_submit').on('click', function (event) {
        ...
        $.ajax({
            url: "<?php echo e(route('tax_type.store')); ?>",
            method: "POST",
    ...
    $(document).on('click', '.tax_edit', function () {
        ...
        var target = "<?php echo e(route('tax_type.index')); ?>/" + id + '/edit';
        $.ajax({
            url: target,
            dataType: "json",
    ...
    $('#tax_type_edit_submit').on('click', function (event) {
        ...
        $.ajax({
            url: "<?php echo e(route('tax_type.update')); ?>",
            method: "POST",
            data: {
                name_edit: name_edit,
                rate_edit: rate_edit,
    ...
    $(document).on('click', '.tax_delete', function () {
        ...
        let target = "<?php echo e(route('tax_type.index')); ?>/" + delete_id + '/delete';
...
<?php /**PATH C:\xampp\htdocs\CRM\resources\views/projects/tax_type/tax_type_js.blade.php ENDPATH**/ ?>